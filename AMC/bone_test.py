...
class Ax:
    ...
    def draw_scatter(self,scatter_list,color):
        for scatter in scatter_list:
            try:
                x, y, z = scatter
                self.ax.scatter(x,y,z,s=8,color=color)
            except:
                x, y = scatter
                self.ax.scatter(x,y,s=8,color=color)
    def clear(self):
        self.ax.cla()
#Magic,don't touch
...
target_fig = plt.figure(num="target pose",figsize=(5,5))
target_ax_3d = Ax(111,target_fig,'3d')
target_ax_3d.set_title('Target Pose')
#骨骼类
class Armature:
    ...
    def rotate(self,target_pose):
        ...
        # write_rotate_angle(1,[pitch,yaw])
        ...
        yaw, pitch = self.calculate_vector_rotation(8,9)
        self.update_pose(yaw, pitch,control_bone=8)
        write_rotate_angle(8,[pitch,yaw])
        yaw, pitch = self.calculate_vector_rotation(11,12)
        ...
        yaw, pitch = self.calculate_vector_rotation(12,13)
...
if __name__ == "__main__":
    ...
    pose_file.close()
    initialize_pose_data = pose_data_list
    initialize_front_plot_list = [list(item) for item in initialize_pose_data[0]['front'][0]['initialize_pose_info']]
    initialize_left_plot_list = [list(item) for item in initialize_pose_data[1]['left'][0]['initialize_pose_info']]
    initialize_right_plot_list = [list(item) for item in initialize_pose_data[2]['right'][0]['initialize_pose_info']]
    ...
    previous_pose_plot_list = initialize_pose_plot_list  # 初始化时将第一个姿势作为上一帧姿势

    """
    下一帧动作的骨骼位置
    """
    frame_count = 1
    front_capture = cv2.VideoCapture('AMC/output/video/New Camera 2.avi')
    left_capture = cv2.VideoCapture('AMC/output/video/New Camera 3.avi')
    right_capture = cv2.VideoCapture('AMC/output/video/New Camera 0.avi')
    frame_counts = int(front_capture.get(cv2.CAP_PROP_FRAME_COUNT))
    print("frame count: ",frame_counts)

    while frame_counts != frame_count:
        front_image = front_capture.read()[1]
        left_image = left_capture.read()[1]
        right_image = right_capture.read()[1]
        ...
        if frame_count % 8 == 0:
            ...
            for image in camera_image:
                # Openpose检测
                next_info = openpose_detector.detect_pose(image)
            ...
            # 基于上一帧的姿势进行旋转，而不是初始姿势
            pose = Armature(previous_pose_plot_list, target_ax_3d)
            pose.rotate(target_pose=target_pose_plot_list)

            # 更新上一帧姿态
            previous_pose_plot_list = target_pose_plot_list.copy()
            ...
            for plot_index, plot in enumerate(target_pose_plot_list):
                if plot_index in [2, 3, 4, 8, 9, 10]:
                    target_ax_3d.draw_scatter([plot], color='blue')
                elif plot_index in [5, 6, 7, 11, 12, 13]:
                    target_ax_3d.draw_scatter([plot], color='red')
                else:
                    target_ax_3d.draw_scatter([plot], color='green')

            print(f"frame_count: {frame_count}")
            #cv2.imshow("show", front_image)
            if frame_count >= 240:
                plt.show(block=True)
            current_ax_3d.clear()
            target_ax_3d.clear()
