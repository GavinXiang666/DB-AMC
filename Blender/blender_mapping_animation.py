...
class Armature: 
    ...
    def rotate(self,target_armature,last_armature=None):
        ...
        if self.first_state == True:
            for n,bone_name in enumerate(['torso','head','upper_arm_fk.L','forearm_fk.L','upper_arm_fk.R',
                ...
                k_data = json_reader('D:/DB-AMC/Blender/K_Data_Kanade.json',bone_name)
...
if __name__ == "__main__":
    ...
    # 设置动画参数
    frame_interval = 0.25  # 帧间隔时间（秒）
    fps = bpy.context.scene.render.fps  # 获取当前场景帧率
    frame_step = int(frame_interval * fps)  # 转换为帧数量
    current_frame = 1  # 起始帧
    
    # 设置初始姿态关键帧
    bpy.context.scene.frame_set(current_frame)
    for bone in angel_rig.pose.bones:
        bone.keyframe_insert(data_path="rotation_euler")
    
    initial_armature_object = armature_object_list[0]

    # 按顺序应用所有相邻姿态的旋转
    for i in range(len(armature_object_list) - 1):
        ...
        # 当前姿态旋转到下一个姿态
        current_armature = armature_object_list[i]
        next_armature = armature_object_list[i + 1]

        if i != 0:
            last_armature = armature_object_list[i - 1]
            current_armature.rotate(next_armature,last_armature)
        else:
            current_armature.rotate(next_armature,last_armature=None)
            
        # 添加关键帧记录旋转
        for depart in ['torso','head','upper_arm_fk.L','forearm_fk.L','upper_arm_fk.R',
                       'forearm_fk.R','thigh_fk.L','shin_fk.L','thigh_fk.R','shin_fk.R']:

            angel_rig.pose.bones[depart].keyframe_insert(data_path="rotation_euler")
        
        print(i)
        print(current_armature.blender_pose)
    # 关闭数据库连接
    conn.close()
